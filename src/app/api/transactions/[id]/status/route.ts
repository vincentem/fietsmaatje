...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (res.rows[0]) {
      await query('INSERT INTO audit_logs (event_type, related_type, related_id, payload) VALUES ($1,$2,$3,$4)', ['transaction.status_changed','transaction', id, { status }]);
      try { await notifyEvent('transaction.status_changed', 'transaction', id, { status }); } catch (e) { console.error(e); }
      const tx = res.rows[0];
      if (tx.user_id) {
        try {
